// pricing.js v50 — RU подписи; матрица доступности; безопасные значения
(function () {
  ...
  const C = {
    ...
    OTP:{code:'OTP',  nameRu:'ОТП',        icon:ICON('bank')}, // временно
  ...
  const MATRIX = {
    // ОТДАЮ
    cash: {
      moscow:   ['RUB','USD'],         // в Москве наличные CNY отдать нельзя
      guangzhou:['RUB','USD'],         // и в Гуанчжоу «отдаю CNY» тоже нельзя
    },
    bank: {
  ...
  // Котировки (примерные; заявка фиксируется на 30 минут фронтом)
  const R = {
    ...
    // Китайские сервисы (через USDT/пример)
  ...
  // страховки от «кривых» значений
  const safeCity = (city) => (city === 'moscow' || city === 'guangzhou') ? city : 'moscow';

  function currencies(kind, city, side){
    const c = safeCity(city);
    if (side === 'from') {
      const key = (kind==='cash'||kind==='bank'||kind==='crypto') ? kind : 'cash';
      return mapCodes((MATRIX[key][c]) || []);
    } else {
      const key = (kind==='bank') ? 'bank_to' : (kind==='crypto') ? 'crypto_to' : (kind==='cnpay') ? 'cnpay_to' : 'cash_to';
      return mapCodes((MATRIX[key][c]) || []);
    }
  }
